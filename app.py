...
def index():
    ...
    agents = User.query.filter(User.role.in_(['admin', 'agent']), User.is_active==True).all()
...
def add_comment(ticket_id):
    ...
    comment = Comment(
        ...
        is_internal=request.form.get('is_internal') == 'on' and user.role in ['admin', 'agent']
    )
...
with app.app_context():
    ...
    if not User.query.first():
        default_users = [
            ...
            User(
                username='ibrahim',
                email='shawady6@gmail.com',
                password_hash=generate_password_hash('password123'),
                full_name='Ibrahim Elshawady',
                role='admin'
            ),
            User(
                username='agent1',
